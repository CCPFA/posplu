...
//////end DOM creations
	...
	_defaultFilterData: function(text, records) {
		...
		text = text.replace(/[.*+?^${}()|[\]\\]/g, '')
    text = text.replace(/[eéèêë]/g,"e");
    text = text.replace(/[aàäâ]/g,"a");
    text = text.replace(/[iîï]/g,"i");
    text = text.replace(/[uùüû]/g,"u");
    text = text.replace(/[oöô]/g,"o");
    text = text.replace(/[OÖÖ]/g,"O");
    text = text.replace(/[EÉÈÊË]/g,"E");
    text = text.replace(/[AÀÂÄ]/g,"A");
    text = text.replace(/[IÏÎ]/g,"I");
    text = text.replace(/[UÙÜÛ]/g,"U");
    //text = text.replace("E","[EÉÈÊË]").replace("É","[EÉÈÊË]").replace("È","[EÉÈÊË]").replace("Ê","[EÉÈÊË]").replace("Ë","[EÉÈÊË]");  //sanitize remove all special characters
		if(text==='')
			return [];
      ...
      key0= obj = JSON.parse(JSON.stringify(key));
      key = key.replace("œ","oe");
       key = key.replace(/[eéèêë]/g,"e");
       key = key.replace(/[aàäâ]/g,"a");
       key = key.replace(/[iîï]/g,"i");
      key = key.replace(/[uùüû]/g,"u");
      key = key.replace(/[oöô]/g,"o");
      key = key.replace(/[OÖÖ]/g,"O");
      key = key.replace(/[EÉÈÊË]/g,"E");
      key = key.replace(/[AÀÂÄ]/g,"A");
      key = key.replace(/[IÏÎ]/g,"I");
      key = key.replace(/[UÙÜÛ]/g,"U");
			if( regSearch.test(key))
				frecords[key0]= records[key0];
		}
